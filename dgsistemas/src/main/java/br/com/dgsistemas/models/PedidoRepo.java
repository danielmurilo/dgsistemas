...
public interface PedidoRepo extends CrudRepository<Pedido, Long> {
	...
	@Query(value = "SELECT IFNULL(SUM(p.valor_venda),0.00) AS 'totalDinheiro' FROM pedido p WHERE p.produto_id = 1 AND DATE(p.data) = :date", 
			  nativeQuery = true)
	public Double valorTotalVendasEmDinheiroPorData(@Param("date") Date date);
	
	@Query(value = "SELECT IFNULL(SUM(p.valor_venda),0.00) AS 'totalCartao' FROM pedido p WHERE p.produto_id = 2 AND DATE(p.data) = :date", 
			  nativeQuery = true)
