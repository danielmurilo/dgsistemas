...
public interface ContaRepo extends CrudRepository<Conta, Long>{
	...
	@Query(value = "SELECT c.id_conta, c.data_abertura, LEFT(c.nome_mesa, 10) AS nome_mesa, c.status, c.funcionario_id, c.id_conta, (SELECT COALESCE(SUM(p.valor_venda*p.qtd), 0.00) FROM pedido p WHERE p.conta_id = c.id_conta) as total FROM conta c WHERE c.status = 1", 
			  nativeQuery = true)
	public List<Conta> listarContasComTotal();	
	
	@Query(value = "SELECT c.id_conta, c.data_abertura, c.nome_mesa, c.status, c.funcionario_id, (SELECT COALESCE(SUM(p.valor_venda*p.qtd), 0.00) FROM pedido p WHERE p.conta_id = c.id_conta) AS total FROM conta c WHERE c.id_conta = :id", 
			  nativeQuery = true)
